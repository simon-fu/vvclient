...
macro_rules! init_log {
    () => {
        $crate::log::init_log_with($crate::env_filter!()?, None, None)
    };

    ($extra_mods:expr) => {
        $crate::log::init_log_with($crate::env_filter!($extra_mods)?, None, None)
    };

    ($extra_mods:expr, $file_args:expr) => {
        $crate::log::init_log_with($crate::env_filter!($extra_mods)?, $file_args, None)
    };

    ($extra_mods:expr, $file_args:expr, $to_std_out:expr) => {
        $crate::kit::log::init_log_with($crate::env_filter!($extra_mods)?, $file_args, $to_std_out)
    };
}
...
pub fn make_file_layer<T, S>(
    file_args: &LogFileArgs,
    timer: T,
) -> Result<(
    ...
    S: tracing::Subscriber + for<'a> tracing_subscriber::registry::LookupSpan<'a>,
{
    ...
    let filename_prefix: Cow<str> =
        if file_args.rolling == tracing_appender::rolling::Rotation::NEVER {

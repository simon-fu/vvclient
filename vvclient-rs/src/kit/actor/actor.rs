...
impl<E: ActorHandler> ActorBuilder<E> {
    ...
    pub fn build(self, span: tracing::Span, entity: E) -> Actor<E>
    where
        E: ActorHandler,
    {
        // let (op_tx, op_rx) = mpsc::channel(128);
...
impl<E: ActorHandler> Invoker<E> {
        ...
        E: AsyncHandler<Request, Response = Response> + Send,
    {
        ...
        let rsp = rx
            .await
            .with_context(|| "recv actor invoke response but error")?;
        Ok(rsp)
    }
    ...
    E: ActorHandler,
{
    match op {
        ...
        Op::Msg(msg) => return entity.handle_msg(msg).await,
    }
}

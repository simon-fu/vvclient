...
impl CertBytes {
    pub async fn load_fils(key_path: &Path, cert_path: &Path) -> Result<Self> {
        Ok(Self {
            key_bytes: tokio::fs::read(key_path)
                .await
                .with_context(|| format!("failed to load key file [{key_path:?}]"))?,
            key_ext: Self::path_ext_string(key_path),

            cert_bytes: tokio::fs::read(cert_path)
                .await
                .with_context(|| format!("failed to load cert file [{cert_path:?}]"))?,

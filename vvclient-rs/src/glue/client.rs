...
use crate::{
    client::{defines::JoinConfig, xfer::Xfer},
    kit::astr::AStr,
    proto,
};
...
#[uniffi::export]
#[trace_result]
pub fn make_signal_client(
    url: String,
    config: records::SignalConfig,
    listener: Arc<dyn Listener>,
) -> GlueResult<SignalClient> {
...
impl SignalClient {
    ...
    pub async fn leave_room_and_wait(&self, end: Option<bool>, force: bool) {
        ...
        match r {
            Ok(()) => {
                ...
                let r = rx.await;
                debug!(
                    "leave_room_and_wait: on_closed received, is_ok {}",
                    r.is_ok()
                );
            }
            Err(e) => {
                ...
                warn!(
                    "leave_room_and_wait: commit_op failed: {}",
                    trace_fmt!("leave failed", e)
                );
            }
        }
    }
    ...
    pub fn upub_stream(
        &self,
        req: records::UPubCall,
        cb: Arc<dyn OnResolveUPub>,
    ) -> GlueResult<()> {
        self.simple_request(req, move |rsp| cb.resolve(rsp))?;
        Ok(())
    }
    ...
    pub fn mute_stream(
        &self,
        req: records::MuteCall,
        cb: Arc<dyn CbResolveMute>,
    ) -> GlueResult<()> {
        self.simple_request(req, move |rsp| cb.resolve(rsp))?;
        Ok(())
    }
    ...
    pub fn sub_stream(&self, req: records::SubCall, cb: Arc<dyn CbResolveSub>) -> GlueResult<()> {
        self.commit_op(Op::Request(Box::new(move |delegate, session| {
            let user = delegate
                ...
                .with_context(|| format!("Not found user"))?;

            let x_id = delegate
                .dlink_x_id
                .as_ref()
                .with_context(|| "no dlink_x_id")?
                .clone();

            let (stream_id, stream) = user
                .state
                .streams
                .iter()
            ...
            delegate.response_handlers.insert(
                sn,
                Box::new(move |_delegate, response| {
    ...
    pub fn unsub_stream(
        &self,
        req: records::UnsubCall,
        cb: Arc<dyn CbResolveUSub>,
    ) -> GlueResult<()> {
        self.commit_op(Op::Request(Box::new(move |delegate, session| {
...
impl<L: Listener> DelegateImpl<L> {
    ...
    async fn process_op(&mut self, session: &mut Session, msg: Op<L>) -> Result<()> {
        match msg {
            Op::Leave { end, tx } => {
                debug!(
                    "process_op: Op::Leave start end={:?} tx_present={}",
                    end,
                    tx.is_some()
                );
    ...
    fn req_create_x(&mut self, xfer: &mut Xfer, dir: i32) -> Result<()> {
        ...
        self.send_request(
            xfer,
            &body,
            Box::new(move |delegate, response| {
                ...
                delegate
                    .listener
                    .on_created_x(OnCreatedXArgs { dir, response: rsp })?;
    ...
    fn req_batch_end(&mut self, xfer: &mut Xfer) -> Result<()> {
        let body = proto::BatchEndRequestSer {}.into_body();

        self.send_request(xfer, &body, Box::new(move |_delegate, _response| Ok(())))?;
    ...
    fn req_leave(
        ...
        debug!(
            "req_leave: request queued, storing leave_tx {}",
            tx.is_some()
        );
    ...
    async fn handle_server_push(
        ...
        match push.typ {
            proto::ServerPushType::Closed(notice) => return Err(notice.status.unwrap_or_default()),
            ...
            proto::ServerPushType::UInit(id) => {
                ...
                if let Some(cell) = self.users.remove(user_id.as_str()) {
                    ...
                    self.listener.on_user_leaved(OnUserLeavedArgs { user_id })?;
                }
            }
                        ...
                        match &cell.stage {
                            ...
                            UserStage::Ready => {
                                ...
                                if new_state.inst_id != cell.state.inst_id {
                                    // 不应该走到这里
                                    warn!(
                                        "got new user instance, old [{}], new [{}]",
                                        cell.state.inst_id, new_state.inst_id
                                    );
                                    self.listener.on_user_leaved(OnUserLeavedArgs {
            ...
            proto::ServerPushType::UTree(mut tree_state) => {
                let user_id = tree_state
                    .id
                    .take()
                    .with_context(|| "no user_id in user tree")?;
...
impl<L: Listener> Delegate for DelegateImpl<L> {
    ...
    async fn on_closed(&mut self, status: proto::Status) -> Result<()> {
        ...
        debug!(
            "recv on_closed, status [{:?}], leave_tx {}",
            status,
            self.shared.has_leave_tx()
        );
    ...
    async fn on_response(
        ...
        session: &mut Session,
        ack: i64,
        mut response: proto::ServerResponse,
    ) -> Result<()> {
        let resolver = self
            .response_handlers
            .remove(&ack)
        ...
        resolver(
            self,
            response.typ.with_context(|| "no typ field in response")?,
        )?;
...
type RequestBuilder<L> =
    Box<dyn FnOnce(&mut DelegateImpl<L>, &mut Session) -> Result<()> + Send + Sync>;
type ResponseResolver<L> = Box<
    dyn FnOnce(&mut DelegateImpl<L>, proto::response::ResponseType) -> Result<()> + Send + Sync,
>;

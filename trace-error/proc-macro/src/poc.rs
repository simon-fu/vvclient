        ...
        if let Some(last) = new_block.stmts.pop() {
            match last {
                ...
                Stmt::Expr(expr, None) => {
                    ...
                    //     dbgd!("fold_block: expr ok [{}]", quote!{#expr});
                    ...
                    //     // dbgd!("fold_block: expr [{}], new_tokens [{}]", quote!{#expr}, new_tokens);
...
fn attach_expr(expr: &Expr, fn_name: &Ident, span: proc_macro2::Span) -> syn::Result<Expr> {
    let new_tokens = quote_spanned! { span =>
        ( #expr ).with_context(|| concat!(
            "at ", file!(), ":", line!(), ":", column!(), ", ",
            stringify!(#fn_name), "()"
